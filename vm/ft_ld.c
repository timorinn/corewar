...
/*   Updated: 2019/12/14 23:34:12 by bford            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				ft_init_reg(unsigned char map[4096][4], int position,
int arg)
{
	int		dir_or_ind;
...
int				ft_ld(unsigned char map[4096][4], careta *car)
{
	...
	reg = ft_init_reg(map, car[0].position, args[0]);
	...
	if (ft_ld_error(args, reg, car, "1100"))
		return (1);
	mvprintw(54, 200, "DEPbMO! DATA = %u | bits %u %u %u %u", dir.data, dir.bits[0], dir.bits[1], dir.bits[2], dir.bits[3]);
