...
/*   Updated: 2019/12/29 01:36:56 by bford            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_find_color(int position, t_cursor *car, int def)
{
	...
	size = 0;
	while (size < car->size && car[size].position != position)
		++size;
...
int		ft_print_params(t_cursor *car)
{
	...
	while (i >= 0)
	{
		printf("Car #%d   | pl_num: %d | position: %4d | oper: %02x | cd: %3d | carry: %d | size: %2d | live: %5d",
		car[i].num ,car[i].play_num, car[i].position, car[i].operation, car[i].cooldown, car[i].carry, car[i].size, car[i].live);
		reg = 0;
		while (reg < 8 && ++reg)
		{
